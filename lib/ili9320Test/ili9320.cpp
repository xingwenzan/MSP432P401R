...
/*u16 const Ili9320_Pin[ConPinNum] 		= {nCsPin	,RsPin	,nWrPin	,nRdPin	,nRstPin	};
GPIO_TypeDef* Ili9320_PORT[ConPinNum] 	= {nCsPort	,RsPort	,nWrPort,nRdPort,nRstPort	};
u32 const Ili9320_RCC[ConPinNum] 		= {nCsClk	,RsClk	,nWrClk	,nRdClk	,nRstClk	};*/
...
{ 
	...
	pinMode(nRsPin, OUTPUT);
  pinMode(nClkPin, OUTPUT);
  pinMode(nDatPin, OUTPUT);
  pinMode(nRstPin, OUTPUT);
//	u8 m;
//    GPIO_InitTypeDef GPIO_InitStructure;
//
//	for(m = 0;m < ConPinNum; m++)
//		{
//        RCC_APB2PeriphClockCmd(Ili9320_RCC[m], ENABLE);		/*������Ӧʱ�� */
//        GPIO_InitStructure.GPIO_Pin 	= Ili9320_Pin[m];
//        GPIO_InitStructure.GPIO_Speed 	= GPIO_Speed_10MHz; 	
//        GPIO_InitStructure.GPIO_Mode 	= GPIO_Mode_Out_PP;	//	GPIO_Mode_IPU
//        GPIO_Init(Ili9320_PORT[m], &GPIO_InitStructure);
//		}
//	
//	
//	#ifdef nData16Bits
//		RCC_APB2PeriphClockCmd(nDataClk, ENABLE); 	
//		GPIO_InitStructure.GPIO_Pin = nData16Pins;
//		GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
//		GPIO_InitStructure.GPIO_Mode = GPIO_Mode_Out_PP;
//		GPIO_Init(nDataPort, &GPIO_InitStructure);
//	#endif
//	#ifdef nData8Bits
//		RCC_APB2PeriphClockCmd(nDataClk, ENABLE); 	
//		GPIO_InitStructure.GPIO_Pin = nData8Pins;
//		GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
//		GPIO_InitStructure.GPIO_Mode = GPIO_Mode_Out_PP;
//		GPIO_Init(nDataPort, &GPIO_InitStructure);
//	#endif
...
{
	#ifdef nData16Bits
		GPIO_Write(nDataPort,data);
		Clr_nWr;Set_nWr;
	#endif
	#ifdef nData8Bits
		u16 m;
		m = GPIO_ReadOutputData(nDataPort);
		m = (data>>8)|(m&0xff00);
		GPIO_Write(nDataPort,m);
		Clr_nWr;Set_nWr;
		m = GPIO_ReadOutputData(nDataPort);
		m = (data&0x00ff)|(m&0xff00);
		GPIO_Write(nDataPort,m);
		Clr_nWr;Set_nWr;
	#endif
...
/*	GPIO_InitTypeDef GPIO_InitStructure;
		...
		Clr_nRd;     		
		tmp = GPIO_ReadInputData(nDataPort);
		Set_nRd;
		...
		Clr_nRd;     		
		tmp = GPIO_ReadInputData(nDataPort)<<8;
		Set_nRd;
		
		Clr_nRd;     		
		tmp |= GPIO_ReadInputData(nDataPort)&0x00ff;
		Set_nRd;
